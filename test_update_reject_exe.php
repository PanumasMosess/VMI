	...
	if(($lastname == "xls") || ($lastname == "xlsx"))
	{
		...
		*/
		...
		foreach ($namedDataArray as $result)
		{
			...
			$strSQL_del1 = "UPDATE [tbl_replenishment] set [repn_conf_status] = 'Reject'
			WHERE [repn_order_ref] = '".$result["Ref_No"]."'
            AND [repn_fg_code_set_abt] = '".trim($result["FG_Code_Set"])."'
            AND [repn_sku_code_abt] = '".trim($result["Component_Code"])."'
            AND [repn_part_customer] = '".trim($result["Part_Customer"])."'
            AND [repn_fg_code_gdj] = '".trim($result["FG_Code_GDJ"])."'
            AND [repn_qty] = '".trim($result["Quantity"])."'
            AND [repn_terminal_name] = '".trim($result["Terminal_Name"])."'
            AND [repn_by] = '".trim($result["Issue_By"])."'
			";
