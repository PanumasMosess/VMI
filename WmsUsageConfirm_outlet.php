	...
	<div class="wrapper">
		...
		<div class="content-wrapper">
			...
			<section class="content-header">
				<h1><i class="fa fa-caret-right"></i>&nbsp;ขายสินค้า<small>สแกน Tags ขาย</small></h1>
				<ol class="breadcrumb">
					<li><a href="<?= $CFG->wwwroot; ?>/home"><i class="fa fa-home"></i>Home</a></li>
					<li class="active">Usage Confirm (Pick)</li>
				</ol>
			</section>
					...
					<div class="col-xs-12">
						<div class="box box-info">
							<div class="box-header with-border">
								<h3 class="box-title"><i class="fa fa-cubes"></i> Stock Blance Outlet Store <small>
										<font style="color: red; font-size:12px;">*Stock ล่าสุดที่อยู่ใน outlet store</font>
									</small></h3>
							</div>
							<!-- /.box-header -->
						...
						<div class="box box-warning">
							<div class="box-header with-border">
								<h3 class="box-title"><i class="glyphicon glyphicon-qrcode"></i> ขายสินค้า <small>
										<font style="color: red; font-size:12px;">*หน้าร้านขายสินค้าในส่วนนี้</font>
									</small></h3>
							</div>
						...
						<select id="txt_usage_conf_pj" name="txt_usage_conf_pj" onchange="_on_load_pj(this.value)" class="c_txt_usage_conf_pj form-control input-sm select2" style="width: 100%">
							<option value="" selected="selected">Select</option>
								...
								<div class="col-md-4"><i class="fa fa-search" height="16px"></i> Select Component Code/FG Code:
									<select id="txt_usage_conf_part_name" name="txt_usage_conf_part_name" onchange="_on_load_part_name(this.value)" class="c_txt_usage_conf_part_name form-control input-md select2" style="width: 100%">
										<option value="" selected="selected">Select</option>
									</select>
								</div>
								...
								<div class="col-md-4"><img src="<?= $CFG->iconsdir; ?>/Ecommerce-Barcode-Scanner-icon.png" height="16px"> Scan Tags:
									<input type="text" id="txt_usage_conf_scan_tags" name="txt_usage_conf_scan_tags" onKeyPress="if (event.keyCode==13){ return _onScan_TagsID(); }" class="form-control input-md" placeholder="Scan Tags ID." autocomplete="off" autocorrect="off" spellcheck="false">
								</div>
							...
							<div class="box-body table-responsive padding">
								<table id="tbl_tags_usage_conf_pre" class="table table-bordered table-hover table-striped nowrap">
									<thead>
										<tr style="font-size: 17px;">
											<th colspan="8" class="bg-warning"><b><i class="glyphicon glyphicon-qrcode"></i>&nbsp;Usage confirm tags list</b> <small>
													<font style="color: red; font-size:12px;">*เมื่อสแกน Tags ตามที่ต้องการแล้ว ให้กดปุ่ม Confirm เพื่อยืนยันการขาย</font>
												</small><span class="pull-right"><button type="button" class="btn btn-primary btn-sm" onclick="_usageConfirmPre()"><i class="fa fa-check"></i> Summary</button> | <button type="button" class="btn btn-danger btn-sm" onclick="_DelUsageConfirmPre()"><i class="fa fa-trash"></i> Delete</button></span></th>
										</tr>
										<tr style="font-size: 13px;">
											<th style="width: 30px;">No.</th>
		...
		<!--------------------dlg tags-calculate-------------------->
<div class="modal fade" id="modal-cal-customer" data-keyboard="false" data-backdrop="static">
		...
		<div class="modal-content">
			...
			<div class="modal-body">
				<div class="box-body">
					...
					<div class="form-group">
						<label for="customer_tel"> เบอร์โทรศัพท์</label>
						<input type="text" class="form-control" id="customer_tel" maxlength="10" onkeyup="this.value=this.value.replace(/[^\d]/,'')" placeholder="กรอกเบอร์โทรศัพท์ลูกค้า">
					</div>
					<div class="form-group">
						<label for="customer_inv"> เลขใบกำกับภาษี <font style="color: #dd4b39;"> *ถ้าต้องการ</font></label>
						<input type="text" class="form-control" id="customer_inv" maxlength="15" placeholder="เลขใบกำกับภาษี">
					</div>
					<div class="row">
						<div class="col-lg-4">
							<div class="form-group">
								<label for="customer_price"> ลูกค่าจ่ายเงิน</label>
								<input type="number" class="form-control" id="customer_price" maxlength="10" placeholder="กรอกจำนวนเงิน">
							</div>
						</div>
						<div class="col-lg-4">
							<div class="form-group">
								<label for="price_sum"> ราคารวม</label>
								<input type="text" class="form-control" id="price_sum" maxlength="10" disabled>
							</div>
						</div>
						<div class="col-lg-3">
							<div class="form-group">
								<label for="customer_change"> เงินทอน</label>
								<input type="text" class="form-control" id="customer_change" maxlength="10" disabled>
							</div>
						</div>
					</div>
				</div>
			...
			<div class="modal-footer">
				<button type="button" class="btn btn-success btn-sm" onclick="_usageConfirmCon();">Confirm</button>
				<button type="button" class="btn btn-default btn-sm" onclick="_closeModal()" data-dismiss="modal">Close</button>
			</div>
		</div>
		<!-- /.modal-content -->
	</div>
	...
	<script language="javascript">
		...
		function _usageConfirmPre() {
			...
			if (arr_length == 0) {
				...
				$('#modal-cal-customer').modal('show');

				$.ajax({
					type: 'POST',
					url: '<?= $CFG->src_wms_usage_conf; ?>/_load_detail_by_tags.php',
					data: {
						var_arr_list: arr_list
					},
					success: function(response) {
						var product_detail = JSON.parse(response);
						var sumprice_customer = 0;
						product_detail.map(func_get_price);

						function func_get_price(value, index) {
							sumprice_customer += (value.tags_packing_std * value.bom_price_sale_per_pcs);
						}
						$('#price_sum').val(sumprice_customer);
					},
					error: function() {
						//dialog ctrl
						swal({
		...
		function _usageConfirmCon() {
			...
			if (arr_length == 0) {
				...
				swal({
					html: true,
					title: "<span style='font-size: 15px; font-weight: bold;'>Warning !!!</span>",
					text: "<span style='font-size: 15px; color: #000;'>โปรดเลือกอย่างน้อย 1 รายการ</span>",
					type: "warning",
					timer: 2000,
					showConfirmButton: false,
					allowOutsideClick: false
				...
				swal({
					html: true,
					title: "<span style='font-size: 15px; font-weight: bold;'>Warning !!!</span>",
					text: "<span style='font-size: 15px; color: #000;'>กรุณากรอกชื่อลูกค้า</span>",
					type: "warning",
					timer: 2000,
					showConfirmButton: false,
					allowOutsideClick: false
				...
				swal({
					html: true,
					title: "<span style='font-size: 15px; font-weight: bold;'>Warning !!!</span>",
					text: "<span style='font-size: 15px; color: #000;'>กรุณากรอกเบอร์โทรศัพท์</span>",
					type: "warning",
					timer: 2000,
					showConfirmButton: false,
					allowOutsideClick: false
			...
			} else if ($('#customer_price').val() == '') {
				//dialog ctrl
				swal({
					html: true,
					title: "<span style='font-size: 15px; font-weight: bold;'>Warning !!!</span>",
					text: "<span style='font-size: 15px; color: #000;'>กรุณากรอกจำนวนเงิน</span>",
					type: "warning",
					timer: 2000,
					showConfirmButton: false,
					allowOutsideClick: false
			...
			$.ajax({
			  type: 'POST',
			  url: '<?= $CFG->src_wms_usage_conf; ?>/_UsageConfirm_outlet.php',
			  data: {
						var_arr_list: arr_list,
						customer_name: $('#customer_name').val(),
						customer_tel: $('#customer_tel').val(),
						product_price_sum: $('#price_sum').val(),
						customer_price: $('#customer_price').val(),
						customer_inv: $('#customer_inv').val()
					},
					success: function(response){
		...
		var input_customer_price = document.getElementById('customer_price');
		input_customer_price.addEventListener('keyup', function(event) {
			...
			if (event.keyCode === 13) {
				...
				var customer_change_ = customer_price - price_sum;

				var customer_changeA = new Intl.NumberFormat('en-IN', {
					maximumSignificantDigits: 3
				}).format(customer_change_)

				console.log(customer_changeA);

				$('#customer_change').val(customer_changeA);
			}
		});
		...
		window.open("<?= $CFG->src_mPDF; ?>/print_billing_outlet?tag=" + id + "", "_blank");
		}
