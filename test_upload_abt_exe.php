	...
	if(($lastname == "xls") || ($lastname == "xlsx"))
	{
		...
		*/
		...
		foreach ($namedDataArray as $result)
		{
			...
			$strSQL_del1 = "INSERT INTO [ABT_RUN].[dbo].[tbl_user_running]
			(
				[abt_user_name]
      		   ,[abt_qr_code]
      		   ,[abt_email]
	  ...
	  VALUES
			(
			 '".trim($result["abt_user_name"])."'
			,'".trim($result["abt_qr_code"])."'
			,'".trim($result["abt_email"])."'
			,'".trim($result["abt_gender"])."'
			,'".trim($result["abt_phone_number"])."'
			,'".trim($result["abt_datetime_start"])."'
			,'".trim($result["abt_datetime_finish"])."'
			,'".trim($result["abt_status"])."'
			,'".trim($result["abt_group_user"])."'
			,'".trim($result["abt_age"])."'
			)";
			$objQuery_del1 = sqlsrv_query($db_con_run, $strSQL_del1);
