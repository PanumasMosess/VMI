...
$str_usr_allow = array('chitnarong','marisa','panumas','pattra','itsara', 'Rangsima', 'GDJ00202');
...
<div class="wrapper">
  ...
  <div class="content-wrapper">
    ...
    <section class="content-header">
      <h1><i class="fa fa-caret-right"></i>&nbsp;WMS Tools<small>เครื่องมือ สำหรับจัดการงาน Fulfillment </small></h1>
      <ol class="breadcrumb">
        <li><a href="<?=$CFG->wwwroot;?>/home"><i class="fa fa-home"></i>Home</a></li><li class="active">WMS Tools</li>
      </ol>
    </section>
...
function _load_pallet_adjust_inventory()
{
	//Load data
	setTimeout(function(){
		//$("#spn_load_data_main").html(""); //clear span
		$("#spn_load_data_main").load("<?=$CFG->src_put_away;?>/load_adjust_stock_pallet.php");
	},300);
}
...
function _onScan_PalletID_adjustStock()
{
	if($("#txt_move_scn_pallet_id_adjustStock").val() != "")
	{
		if(isEnglishchar($("#txt_move_scn_pallet_id_adjustStock").val())==false)
		{
			//dialog ctrl
			swal({
			  html: true,
			...
			//hide
			setTimeout(function(){
				$("#txt_move_scn_pallet_id_adjustStock").val('');
				$("#txt_move_scn_pallet_id_adjustStock").focus();
			}, 2500);
		...
		else
		{
			...
			$.ajax({
			  type: 'POST',
			  url: '<?=$CFG->src_put_away;?>/insert_pre_adj_inventory_pallet.php',
			  data: { 
						iden_txt_move_scn_pallet_id_adjustStock: $("#txt_move_scn_pallet_id_adjustStock").val()
					},
					success: function(response){
					...
					if(response == "duplicate")
					{
						...
						swal({
						  html: true,
						  title: "<span style='font-size: 15px; font-weight: bold;'>Warning !!!</span>",
						  text: "<span style='font-size: 15px; color: #000;'>[C003] --- Tags ID: <b>"+ $("#txt_move_scn_pallet_id_adjustStock").val() +"</b> is already exists !</span>",
						  type: "warning",
						  timer: 1000,
						  showConfirmButton: false,
						  allowOutsideClick: false
					...
					else if(response == "not match")
					{
						...
						swal({
						  html: true,
						  title: "<span style='font-size: 15px; font-weight: bold;'>Warning !!!</span>",
						  text: "<span style='font-size: 15px; color: #000;'>[C003] --- Tags ID: <b>"+ $("#txt_move_scn_pallet_id_adjustStock").val() +"</b> pattern not match ! <br> This Tags ID is not data available in Master Tags ?</span>",
						  type: "error",
						  timer: 1000,
						  showConfirmButton: false,
						  allowOutsideClick: false
					...
					else if(response == "not receive")
					{
						...
						swal({
						  html: true,
						  title: "<span style='font-size: 15px; font-weight: bold;'>Warning !!!</span>",
						  text: "<span style='font-size: 15px; color: #000;'>[C003] --- Tags ID: <b>"+ $("#txt_move_scn_pallet_id_adjustStock").val() +"</b> not receive to inventory !</span>",
						  type: "error",
						  timer: 1000,
						  showConfirmButton: false,
						  allowOutsideClick: false
...
function _remove_adjust_stock_pallet()
{
	//check No data available in table
	if($("#hdn_row_adj_stock").val() == 0)
	{
...
function _confirm_adjust_stock_pallet()
{
	if($("#hdn_row_adj_stock").val() == 0)
	{
		//dialog ctrl
