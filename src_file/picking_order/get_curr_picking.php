...
SELECT 
  ...
  ,picking_code
  FROM tbl_picking_running 
  where picking_status = 'Waiting' and picking_issue_date = '$buffer_date' and picking_issue_by = '$t_cur_user_code_VMI_GDJ' order by picking_id desc
";
...
while($objResult_get_pickCode = sqlsrv_fetch_array($objQuery_get_pickCode, SQLSRV_FETCH_ASSOC))
{
	...
	$picking_code = $objResult_get_pickCode['picking_code'];
}
...
{
	//case picking code status = Waiting
	if($num_row_get_pickCode != 0) //current picking no reserved by current user account
	{
		$sum_pickCode = $buffer_substr_run_digit;
		$sprintf_pickCode = sprintf("%04d",$sum_pickCode);//generate to 4 digit
		$full_pickCode = $buffer_substr_ps.$buffer_substr_ymd.$sprintf_pickCode;//full pickCode
...
$('#txt_curr_picking_no').val('<?=$full_pickCode;?>');
$('#hdn_curr_picking_no').val('<?=var_encode($full_pickCode);?>');
</script>
<?
sqlsrv_close($db_con);
?>