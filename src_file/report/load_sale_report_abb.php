	...
	<table id="tbl_dtn_sheet" class="table table-bordered table-hover table-striped nowrap">
		<thead>
			<tr style="font-size: 13px;">
				<th colspan="21" class="bg-light-blue"><b><i class="fa fa-bar-chart fa-lg"></i>&nbsp;DTN B2C Sale ABB Report</b>&nbsp;<button type="button" class="btn btn-default btn-sm" onclick="exportExcelSaleB2C_ABB()"><i class="fa fa-bar-chart fa-lg"></i> Export Excel</button></th>
			</tr>
			<tr style="font-size: 13px;">
...
SELECT [b2c_sale_date]
  ...
  left join tbl_installment
  on tbl_installment.order_ref = tbl_b2c_detail.b2c_repn_order_ref
...
";
			...
			while ($objResult_DTNSheet = sqlsrv_fetch_array($objQuery_sale_report, SQLSRV_FETCH_ASSOC)) {
				...
				$b2c_sale_id = $objResult_DTNSheet['b2c_sale_id'];
				$b2c_sale_time = $objResult_DTNSheet['b2c_sale_time'];
				$b2c_dtn = $objResult_DTNSheet['b2c_dtn'];
				...
				$installment = $objResult_DTNSheet['installment'];
				$price = $objResult_DTNSheet['price'];
				...
				if (($b2c_status == null) || ($b2c_dtn == "")) {
					$b2c_status_tags  = 'Production Process';
				} else {
					$b2c_status_tags = $b2c_status;
				}
				...
				if ($installment != null) {
					$installment = "งวดชำระที่ " . $installment;
				} else {
					$installment  = '-';
				}
			...
			?>
				<tr style="font-size: 13px;">
					<td><?= $row_id_report; ?></td>
					<? if ($b2c_sale_inv_no == '') { ?>
						<td><input type="text" id="<?= $b2c_sale_id; ?>" onkeypress="Chceck(event, this.id, this.value)"></td>
					<? } else { ?>
...
<!--alert no item-->
<input type="hidden" name="hdn_row_DTNSheet" id="hdn_row_DTNSheet" value="<?= $row_id_report; ?>" />
