...
$objPHPExcel->getActiveSheet()->setCellValue('A1', "Report Daily Plan On $buffer_datetime");
...
$objPHPExcel->getActiveSheet()->setCellValue('A2', "Plan Date");
$objPHPExcel->getActiveSheet()->setCellValue('B2', "Plan ft2");
...
//get data
$strSql_d = " 
SELECT * FROM tbl_daily_plan where CAST(plan_date AS DATE) >= CAST( getdate() AS DATE)
";
...
while($objResult_d = sqlsrv_fetch_array($objQuery_d, SQLSRV_FETCH_ASSOC))
{
	...
	$plan_id = $objResult_d['plan_id'];
	$plan_date = $objResult_d['plan_date'];
	$plan_ft2_value = $objResult_d['plan_ft2_value'];
	$plan_issue_by = $objResult_d['plan_issue_by'];
	$plan_issue_date = $objResult_d['plan_issue_date'];
	$plan_issue_time = $objResult_d['plan_issue_time'];
	$plan_issue_datetime = $objResult_d['plan_issue_datetime'];
	...
	*/	
	...
	$objPHPExcel->getActiveSheet()->setCellValue('A' . $i, $plan_date);
	$objPHPExcel->getActiveSheet()->setCellValue('B' . $i, $plan_ft2_value);
	$objPHPExcel->getActiveSheet()->setCellValue('C' . $i, $plan_issue_by);
	$objPHPExcel->getActiveSheet()->setCellValue('D' . $i, $plan_issue_date);
	$objPHPExcel->getActiveSheet()->setCellValue('E' . $i, $plan_issue_time);
	$objPHPExcel->getActiveSheet()->setCellValue('F' . $i, $plan_issue_datetime);
}

// Rename sheet 
$objPHPExcel->getActiveSheet()->setTitle('Report Daily Plan');

$objPHPExcel->setActiveSheetIndex(0);
// Redirect output to a client’s web browser (Excel5)
header('Content-Type: application/vnd.ms-excel');
header('Content-Disposition: attachment;filename="Report Daily Plan ('.$buffer_datetime.').xls"');
