...
if($fileupload)
{
	...
	if(($lastname == "xls") || ($lastname == "xlsx"))
	{
		...
		{
			//var cehck 
			$str_cus_code = strlen($result_check["Customer Code"]);
			
			//check cus code 
			if($str_cus_code > 4)
			{
				$tigger_digit_check = $tigger_digit_check + 1;
			}
		}
		
		//OK
		if($tigger_digit_check == 0)
		{
			foreach ($namedDataArray as $result)
			{
				...
				$str_ship_type = strtoupper($result["Ship to Type"]);
				$str_part_customer = strtoupper($result["Part Customer"]);

				//Process Insert / update to database
				//Check dulplicate
				$strSQL_Check = " select * from tbl_bom_mst where [bom_fg_code_set_abt] = '$str_fg_code_cus' and [bom_fg_sku_code_abt] = '$str_component_code_cus' and [bom_fg_code_gdj] = '$str_fg_code_gdj' and [bom_pj_name] = '$str_pj_name' and [bom_ship_type] = '$str_ship_type' and [bom_part_customer] = '$str_part_customer' ";
				...
				{  
					$strSQL_update = " UPDATE [dbo].[tbl_bom_mst]
						 ...
						 WHERE [bom_fg_code_set_abt] = '$str_fg_code_cus' and [bom_fg_sku_code_abt] = '$str_component_code_cus' and [bom_fg_code_gdj] = '$str_fg_code_gdj' and [bom_pj_name] = '$str_pj_name' and [bom_ship_type] = '$str_ship_type' and [bom_part_customer] = '$str_part_customer' ";
