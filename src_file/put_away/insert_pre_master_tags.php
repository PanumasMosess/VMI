...
where 
tags_token = '$iden_txt_scan_master_tags'
";
$objQuery_match = sqlsrv_query($db_con, $strSQL_match);
// $result_match = sqlsrv_fetch_array($objQuery_match, SQLSRV_FETCH_ASSOC);

if($objQuery_match)//check match
{
	while($objResult_tags_detail= sqlsrv_fetch_array($objQuery_match, SQLSRV_FETCH_ASSOC)){

		$tags_code   = $objResult_tags_detail['tags_code'];

		//check duplicate pre-receive
		$strSQL_dup_pre_rec = "select pre_receive_tags_code from tbl_pre_receive 
		where 
		pre_receive_tags_code = '$tags_code'
		";
