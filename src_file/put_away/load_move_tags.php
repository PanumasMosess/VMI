...
<div class="box-header with-border">
	<div class="col-md-4"><i class="fa fa-qrcode"></i> Scan Tags ID.: <font style="color: red; font-size:12px;">* สแกน Tags ID ที่ต้องการย้าย</font><input type="text" id="txt_move_scn_tag_id" name="txt_move_scn_tag_id" onKeyPress="if (event.keyCode==13){ return _onScan_TagsID(); }" class="form-control input-sm" placeholder="Scan Tags ID." autocomplete="off" autocorrect="off" spellcheck="false"></div>
	<div class="col-md-4"><i class="fa fa-map-marker"></i> Loacation New.: <font style="color: red; font-size:12px;">Location ที่ต้องการย้าย (ถ้าต้องการ Update)</font><input type="text" id="txt_move_scn_tag_location_move" name="txt_move_scn_tag_location_move" class="form-control input-sm" placeholder="Location." autocomplete="off" autocorrect="off" spellcheck="false"></div>
	<div class="col-md-4"><i class="fa fa-qrcode"></i> Scan Pallet ID. (Destination): <a href="#">
			<font style="color: red; font-size:12px;" onclick="genPallet();">* สแกน Pallet ID ที่จะย้ายไป</font>
		</a><input type="text" id="txt_move_scn_new_pallet" name="txt_move_scn_new_pallet" onKeyPress="if (event.keyCode==13){ return _onScan_TagsID_newPallet(); }" class="form-control input-sm" placeholder="Scan Pallet ID. (Destination)" autocomplete="off" autocorrect="off" spellcheck="false"></div><span id="spn_load_pallet_no"></span>
</div>
<div class="box-body table-responsive padding">
	<table id="tbl_move_tags" class="table table-bordered table-hover table-striped nowrap">
		<thead>
			<tr style="font-size: 18px;">
				<th colspan="9" class="bg-yellow"><b>
						<font style="color: #FFF;"><i class="fa fa-arrows"></i> Move Tags </font>
						<font style="color: #FFF; font-size:12px">(ย้าย Tags ID ไปยัง Pallet อื่นๆ สามารถย้ายได้ครั้งละหลาย Tags )</font>
					</b><span style="float: right;"><button type="button" class="btn btn-default btn-sm" onclick="_remove_pre_move_tags()"><i class="fa fa-trash fa-lg"></i> Clear</button></span></th>
			</tr>
...
<script language="javascript">
	$(document).ready(function() {
		...
		//toUpperCase
		$('#txt_move_scn_tag_id').keyup(function() {
			this.value = this.value.toUpperCase();
		});
		$('#txt_move_scn_new_pallet').keyup(function() {
			this.value = this.value.toUpperCase();
		});
