			...
			if (($lastname == "xls") || ($lastname == "xlsx")) {
				...
				if (($tigger_date_check == 0) and (count($namedDataArray) == $count_data)) {
					foreach ($namedDataArray as $result) {
						...
						$str_customer_company = $result["Customer Company"];
						$str_customer_name = $result["Customer Name"];
						$str_customer_invoice = $result["Customer Invoice"];
						$str_tax_invoice = $result["Tax Invoice"];
						$str_address = $result["Address"]; 
						$str_zipcode = $result["Zip Code"]; 
						$str_tel = $result["Tel"]; 
						$str_sale_vat = $result["Sale Including Vat"]; 
						$case = $result["Case"]; 
						...
						if ($objResult) //Update
						{
							...
							$strSQL_order_detail_update = "
							UPDATE [dbo].[tbl_b2c_detail]
   							SET 
   							    [b2c_customer_code] = '$str_fg_code_cus'
   							   ,[b2c_cus_company] = '$str_customer_company'
   							   ,[b2c_inv_company] = ''
   							   ,[b2c_cus_branch_id] = ''
   							   ,[b2c_customer_name] = '$str_customer_name'
   							   ,[b2c_delivery_address] = '$str_address'
   							   ,[b2c_cus_zipcode] = '$str_zipcode'
   							   ,[b2c_inv_address] = '$str_address'
   							   ,[b2c_zipcode] = ''
   							   ,[b2c_contact_name] = '$str_customer_name'
   							   ,[b2c_tel] = '$str_tel'
   							   ,[b2c_note] = ''
   							   ,[b2c_order_date] = '$buffer_date'
   							   ,[b2c_tax_inv] = '$str_tax_invoice'
   							   ,[b2c_case] = '$case' 
								WHERE b2c_repn_order_ref = '$str_order_ref'
							";
							$objQuery_update_address = sqlsrv_query($db_con, $strSQL_order_detail_update);


							$strSql_order_sale_update = "
							UPDATE [dbo].[tbl_b2c_sale]
  						    SET [b2c_sale_date] = '$buffer_date'
  						       ,[b2c_sale_time] = '$buffer_time'
  						       ,[b2c_sale_pos_no] = 'E052010002A1948'
  						       ,[b2c_sale_inv_no] = '$str_tax_invoice'
  						       ...
  						       ,[b2c_sale_discount_amount] = '0'
                            WHERE b2c_sale_order_id = '$str_order_ref' 
							";
							$objQuery_update_sale = sqlsrv_query($db_con, $strSql_order_sale_update);
